...
class PS_Builder {
    ...
    public function display_template_management() {
        require_once(trailingslashit($this->plugin_dir) . 'email-newsletter-files/page-template-management.php');
    }
    ...
    /**
     * AJAX handler: Save builder settings (index.php)
     */
    public function ajax_save_builder_settings() {
        check_ajax_referer('enewsletter_template_edit', 'nonce');
        ...
        if(empty($stylesheet) || empty($settings)) {
            wp_send_json_error(__('Missing parameters', 'email-newsletter'));
        }
        ...
        $content .= " * Auto-generated by E-Newsletter Template Editor\n";
        $content .= " */\n\n";
        
        // Default colors and images
        $content .= "// Default Colors\n";
        $content .= "define('BUILDER_DEFAULT_BG_COLOR', '" . sanitize_hex_color($settings['bg_color']) . "');\n";
        $content .= "define('BUILDER_DEFAULT_LINK_COLOR', '" . sanitize_hex_color($settings['link_color']) . "');\n";
        $content .= "define('BUILDER_DEFAULT_BODY_COLOR', '" . sanitize_hex_color($settings['body_color']) . "');\n";
        $content .= "define('BUILDER_DEFAULT_ALTERNATIVE_COLOR', '" . sanitize_hex_color($settings['alternative_color']) . "');\n";
        $content .= "define('BUILDER_DEFAULT_TITLE_COLOR', '" . sanitize_hex_color($settings['title_color']) . "');\n\n";
        
        // Default images
        $content .= "// Default Images\n";
        $content .= "define('BUILDER_DEFAULT_BG_IMAGE', '" . sanitize_text_field($settings['bg_image']) . "');\n";
        $content .= "define('BUILDER_DEFAULT_HEADER_IMAGE', '" . sanitize_text_field($settings['header_image']) . "');\n\n";
        
        // Default text
        $content .= "// Default Text\n";
        $content .= "define('BUILDER_DEFAULT_EMAIL_TITLE', '" . sanitize_text_field($settings['email_title']) . "');\n\n";
        
        // Settings
        $content .= "// Template Settings\n";
        $content .= "define('BUILDER_SETTING_USE_DEFAULT_HEADER_FOOTER', " . ($settings['use_default_header_footer'] ? 'true' : 'false') . ");\n";
        $content .= "define('BUILDER_SETTING_USE_DEFAULT_STYLES', " . ($settings['use_default_styles'] ? 'true' : 'false') . ");\n";
        
        // Save file
        if(file_put_contents($index_file, $content)) {
            wp_send_json_success(array('message' => __('Settings saved', 'email-newsletter')));
        } else {
