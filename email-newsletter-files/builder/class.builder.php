...
class Email_Newsletter_Builder  {
	...
	function __construct() {
		...
		if(isset($email_newsletter->settings) && $email_newsletter->settings) {
			...
			add_action( 'wp_ajax_enews_builder_search_items', array( &$this, 'ajax_search_items' ) );
		}
	}
	...
	function enqueue_editor_assets() {
		// Load TinyMCE/Quicktags/media assets early for the Customizer controls iframe
	...
	function create_newsletter($new_values) {
		...
		$default = array(
			...
			'bounce_email' => isset($email_newsletter->settings['bounce_email']) ? $email_newsletter->settings['bounce_email'] : '',
			'sent' => 0,
			'opened' => 0,
			'bounced' => 0,
			'meta' => array('branding_html' => base64_encode(isset($email_newsletter->settings['branding_html']) ? $email_newsletter->settings['branding_html'] : '')),
		);
	...
	public function ajax_search_items() {
		if ( ! current_user_can( 'edit_theme_options' ) && ! current_user_can( 'save_newsletter' ) ) {
			wp_send_json_error( array( 'message' => 'forbidden' ), 403 );
		}

		$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'enews_builder_search_items' ) ) {
		...
		if ( $type === 'product' ) {
			...
			wp_send_json_error( array( 'message' => 'bad_type' ), 400 );
		}
		...
		if ( $term !== '' && preg_match( '/^\d+$/', $term ) ) {
			$query_args['post__in'] = array( (int) $term );
